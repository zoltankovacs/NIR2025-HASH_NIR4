...
expName <- "TempData"					## the name of the experiment
...
###
columnNamesL1 <- c("C_Water", "Y_waterTemp")			## please note that there is a special prefix for class- and numerical variables: the default is "C_" for class-variables and "Y_" for numerical variables
columnNamesL2 <- c("C_DELETE", "C_DELETE")			## "DELETE" is the default character for those columns that, after the generation of the sample list, should be omitted
L1  <- list(list("MQ", "TAP", "DW", "AIR"), list(seq(20, 74, 2))) 					## Please look at the vignette 'Examples for Experiment-Design' for more information on the use of the arguments L1 and L2.
L2  <- L1 
###
Repls <- 1								## how many replications for each sample?
