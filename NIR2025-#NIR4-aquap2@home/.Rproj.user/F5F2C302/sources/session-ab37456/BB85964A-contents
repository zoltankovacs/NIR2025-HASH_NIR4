...
expName <- "20250603_paprika"					## the name of the experiment
...
spacing <- FALSE							## the space between environmental control samples (set to FALSE for not inserting environmental controls)
###
columnNamesL1 <- c("C_Matrix", "C_Adulterant","Y_adultConc")			## please note that there is a special prefix for class- and numerical variables: the default is "C_" for class-variables and "Y_" for numerical variables
columnNamesL2 <- c("C_DELETE", "C_DELETE", "C_DELETE")			## "DELETE" is the default character for those columns that, after the generation of the sample list, should be omitted
L1  <- list(list("paprika"), list("corn"), list(seq(0, 100, 10))) 					## Please look at the vignette 'Examples for Experiment-Design' for more information on the use of the arguments L1 and L2.
L2  <- L1 
###
Repls <- 3								## how many replications for each sample?
Group <- c(expName) 						## additional groups to split the above generated classes into, like e.g. experiment and control
